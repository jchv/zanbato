...
// ComputeStructSize computes the static byte size of a struct from its seq fields.
// Returns -1 if the size cannot be determined statically.
func ComputeStructSize(s *kaitai.Struct) int64 {
	return computeStructSizeHelper(s, 0)
}

func computeStructSizeHelper(s *kaitai.Struct, depth int) int64 {
	if depth > 10 {
	...
	for _, attr := range s.Seq {
		...
		if size < 0 && ref.Kind == types.User {
			...
			size = computeUserTypeSize(s, ref.User.Name, depth+1)
		}
		if size < 0 {
			return -1
		}
...
func computeUserTypeSize(parent *kaitai.Struct, name string, depth int) int64 {
	// Search nested types
	for _, child := range parent.Structs {
		if string(child.ID) == name {
			return computeStructSizeHelper(child, depth)
		}
	}
	return -1
}
